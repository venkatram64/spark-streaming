...
object DailyAverageRevenueAccumulator {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkSession = SparkSession
      ...
      .appName("DailyAverageRevenueAccumulator")
      .getOrCreate()
    ...
    val orderItemsMapAccum = sc.accumulator(0,"ordersItemsMp count")
    ...
    val orderItemsValueAccum = sc.accumulator(0,"reduceByKey value count")
    val orderItems = orderItemsMap
                    .reduceByKey((acc, value) => {
                      orderItemsValueAccum += 1
                      acc + value
                    })
    ...
    println("ordersCompletedAccum ", ordersCompletedAccum)
    println("ordersFilterInvokedAccum ", ordersFilterInvokedAccum)
    println("ordersAccum ", ordersAccum)
    println("orderItemsMapAccum ", orderItemsMapAccum)
    println("orderItemsValueAccum ", orderItemsValueAccum)
